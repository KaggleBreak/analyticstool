...
library("ggplot2")
ggplot() + 
  geom_histogram(data = session_booking_cast, aes(x = booking_time_ratio), binwidth = 0.005)
...
session_bind = session_bind[, c(1:8, 11, 13:17, 19:21)]
head(session_bind, 2)
t(t(colnames(session_bind)))
...
# 필요한것만 들고가자.
t(t(colnames(total)))
total_2 = total[, c(1:6, 13:30, 32, 33)]
head(total_2)
...
Sys.time() - time # 500개 기준 8분

plot(rf_model_1)
summary(rf_model_1)
importance(rf_model_1)
importance = importance(rf_model_1)

# plot 내부 데이터를 df로 변환
plot(rf_model_1)
model_plot_df = as.data.frame(plot(rf_model_1))
head(model_plot_df)

model_plot_df_2 = as.data.frame(rf_model_1$err.rate)
head(model_plot_df_2)

model_plot_df[, "obs"] = 1:nrow(model_plot_df)
model_plot_melt = melt(data = model_plot_df, id.vars = "obs")
head(model_plot_melt)

ggplot() + 
  geom_line(data = model_plot_melt,
            aes(x = obs,
                y = value,
                group = variable,
                colour = variable),
            size = 1.5) + 
  geom_point(data = model_plot_melt[c(250, 1250, 2250), ],
             aes(x = obs,
                 y = value,
                 group = variable,
                 colour = variable),
             size = 5) + 
  geom_hline(yintercept = c(min(model_plot_melt[1:1000, "value"]),
                            min(model_plot_melt[1001:2000, "value"]),
                            min(model_plot_melt[2001:3000, "value"])),
             linetype = 2) + 
  labs(x = "Number of Trees", y = "", title = "Err. Rate of RF_1") + 
...
write.csv(submission_1, "submission_160325_3.csv", row.names = FALSE)
write.csv(importance,   "importance_160325_3.csv", row.names = FALSE)
##################################

submission = read.csv("submission_160325_2.csv", stringsAsFactors = FALSE)
table(submission$country)
